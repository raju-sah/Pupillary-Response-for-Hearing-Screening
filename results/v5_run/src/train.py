...
class DummyTemporalModel(nn.Module):
    """
    Minimal temporal feature model for smoke testing GPU execution.
    Designed with universal CUDA compatibility (Linear projections + pooling).
    """
    ...
    def forward(self, x):
        ...
        # Transpose to (batch_size, seq_len, in_channels) for projection
        x_t = x.transpose(1, 2)
        h = self.relu(self.proj(x_t))
        # Global average pool over sequence length: (batch_size, hidden_dim)
        h_pool = h.mean(dim=1)
...
def run_smoke_test(data_dir: Path, output_dir: Path, seed: int = 42) -> Dict[str, Any]:
    ...
    if torch.cuda.is_available():
        ...
        print(f"  Compute Capability: {gpu_cap}")
        print(f"  CUDA Version: {torch.version.cuda}")
        print(f"  Total Memory: {torch.cuda.get_device_properties(0).total_memory / (1024**3):.2f} GB")
    else:
        print("  Notice: CUDA is NOT available. Running on CPU.")
    ...
    if intermediate_files:
        ...
        print("  Notice: No parquet files found in data_dir. Using synthetic tensor for verification.")
        dummy_input = torch.randn(2, 2, 128, dtype=torch.float32)

    # 3. Model Forward and Backward Pass on Device
    dummy_input = dummy_input.to(device)
    dummy_target = torch.tensor([1] * dummy_input.shape[0], dtype=torch.long).to(device)

    model = DummyTemporalModel(in_channels=2, seq_len=128, hidden_dim=32, num_classes=2).to(device)
    ...
    print(f"Executing backward pass (gradient computation on {device})...")
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
