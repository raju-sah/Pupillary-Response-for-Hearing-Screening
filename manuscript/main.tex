...
\begin{itemize}
    ...
    \end{equation}
\end{itemize}
The grand-average response following this decontamination protocol is depicted in Fig.~\ref{fig:grand_average}.
...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{CNN-Transformer:} Combines a 3-layer 1D convolutional tokenizer with a multi-head self-attention Transformer encoder \cite{vaswani2017attention} (4 heads, 2 layers, feed-forward dimension 128) and temporal pooling.
\end{itemize}
...
As summarized in Table~\ref{tab:task1_results} and Fig.~\ref{fig:dl_vs_classical_roc}, \textbf{CNN-Transformer} achieves the highest screening accuracy across all metrics, attaining an ROC-AUC of 0.844. Paired DeLong non-parametric testing \cite{delong1988comparing} confirmed that CNN-Transformer statistically significantly outperforms the best classical tree ensemble ($Z = 9.48, p < 10^{-15}$) and the single-feature clinical heuristic ($Z = 33.6, p < 10^{-15}$).
...
Analysis of learned attention weights $\alpha_t$ across all out-of-fold trials reveals a prominent attention focus between $t = 0.8\text{s}$ and $t = 2.2\text{s}$ (peaking at $1.35\text{s}$, Fig.~\ref{fig:attention_weights}). This corresponds precisely to the known human autonomic pupillary dilation window, verifying that the deep neural network learned genuine physiological LC-NE dynamics rather than opportunistic artifacts.
...
\caption{Screening classification performance as a function of observation window truncation. Truncating at $t = 1.5$\,s preserves $>98\%$ of maximal screening accuracy, enabling a 50\% reduction in trial duration.}
...
\begin{itemize}
    \item \textbf{Early Latency Truncation:} As shown in Fig.~\ref{fig:latency_truncation}, truncating the trial evaluation window to $t = 1.5\text{s}$ post-stimulus retains $>98\%$ of maximal classification performance (ROC-AUC = 0.744 vs 0.755), enabling a 50\% reduction in overall clinical screening duration.
    ...
    \item \textbf{Optical Video Ellipse Extraction:} Direct least-squares ellipse fitting on raw 30~fps eye video streams achieves Pearson $r = 0.991$ and Spearman $\rho = 0.989$ ($p < 10^{-15}$) against dedicated infrared hardware, as plotted in Fig.~\ref{fig:cv_correlation}.
\end{itemize}
...
\caption{Multimodal fusion benchmark on Dataset C (OpenNeuro ds003690). Tri-modal Cross-Attention Fusion (Pupil + ECG + EEG) yields significant diagnostic performance gains ($\text{ROC-AUC} = 0.812$) over any single-modality baseline.}
\label{fig:multimodal_comparison}
\end{figure}

Evaluating synchronized pupillometry, ECG, and EEG on OpenNeuro ds003690 revealed that cross-attention multimodal fusion achieves an ROC-AUC of 0.812 (Fig.~\ref{fig:multimodal_comparison}), outperforming pupil-only (0.751), ECG-only (0.694), and central EEG-only (0.728) models ($p < 10^{-4}$), demonstrating the complementary diagnostic value of autonomic and cortical auditory responses.
